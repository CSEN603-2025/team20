...
import FacultySidebar3 from "./FacultySidebar3";
...
function FacultyModal({ children, onClose }) {
  return (
    <div className="faculty-modal-overlay" onClick={onClose}>
      <div className="faculty-modal-box" onClick={e => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
...
const FacultyDashboard = () => {
  ...
  return (
    <div>
      ...
      <div className="faculty-bg-header">
        <div className="faculty-header-content">
          <div
            className={`faculty-profile-box ${isCollapsed ? "expanded" : ""}`}
            onClick={toggleDropdown}
          >
            <FaUserCircle size={30} color="#4C3D40" />
            <span className="faculty-profile-name">{user.name}</span>
            {isDropdownOpen && (
              <div className="faculty-dropdown-menu">
                <div className="faculty-dropdown-item" onClick={() => setShowPopup(true)}>
                  Profile
                </div>
                <Link to="/login" className="faculty-dropdown-item">
                  Log Out
                </Link>
              </div>
            )}
          ...
          <>
            <div className="faculty-projects-ui-card" style={{ left: "330px", top: "30px" }}>
              <h3>Student Status</h3>
              <p>Track student progress and internship approval status.</p>
            </div>
            <div className="faculty-projects-ui-card" style={{ left: "700px", top: "30px" }}>
              <h3>Company Details</h3>
              <p>View assigned companies and internship roles for each student.</p>
            </div>
            <div className="faculty-projects-ui-card" style={{ left: "1100px", top: "30px" }}>
      ...
      {showPopup && (
        <FacultyModal onClose={() => setShowPopup(false)}>
          ...
          <button className="faculty-close-btn" onClick={() => setShowPopup(false)}>
            Close
          </button>
        </FacultyModal>
      )}
